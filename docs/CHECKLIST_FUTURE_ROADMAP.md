...
| **🔴** | **Progressive Web App (PWA) Support** | Aplikasi masih berjalan sebagai aplikasi web standar. | **Belum Ada**: Butuh konfigurasi file `manifest.json`, `service-worker.js`, push notifications, serta strategi caching offline. |
...
- [ ] Pembuatan `manifest.json` berisi metadata app, icon, dan theme colors.
- [ ] Pembuatan `service-worker.js` untuk caching static assets.
- [ ] Mekanisme deteksi koneksi offline dan penyajian halaman fallback.
- [ ] Instalasi PWA prompt di mobile/desktop browser.
