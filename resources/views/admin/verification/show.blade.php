...
<div class="container-fluid py-4">
    ...
    @else
        <div class="row g-4">
                        ...
                        <div class="row g-4 align-items-center">
                            <div class="col-md-auto text-center">
                                @php 
                                    ...
                                    $foto = $profile->foto ?? ($profile->foto_ktp ?? null); // Fallback to KTP if no profile photo
                                @endphp
                                @if($foto)
                                    <img src="{{ Storage::url($foto) }}" alt="Profile" class="rounded-circle shadow border border-3 border-white object-fit-cover" style="width: 120px; height: 120px;">
                                @else
                        ...
                        <div class="row g-0">
                            <div class="col-md-6 border-end">
                                <div class="p-3 border-bottom">
                                    ...
                                    <span class="fw-bold font-monospace">{{ $instructor->nik ?? ($instructor->instructorProfile->nik ?? '-') }}</span>
                                </div>
                                <div class="p-3">
                                    <small class="text-muted d-block mb-1 text-uppercase fw-bold" style="font-size: 0.65rem;">Tanggal Lahir</small>
                                    <span>{{ $instructor->tanggal_lahir ? (\Carbon\Carbon::parse($instructor->tanggal_lahir)->format('d F Y')) : '-' }}</span>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="p-3 border-bottom">
            ...
            <div class="col-lg-4">
                <!-- Action Buttons -->
                @if($instructor->status === 'pending' || $instructor->verification_status === 'pending')
                ...
                @else
                    <div class="card shadow-sm border-0 mb-4 bg-{{ $instructor->verification_status === 'approved' ? 'success' : 'danger' }} bg-opacity-10">
                        <div class="card-body p-4 text-center">
                            ...
                            <h5 class="fw-bold text-dark mb-1">Status: {{ Str::upper($instructor->verification_status ?? $instructor->status) }}</h5>
                            <p class="text-muted small mb-0">Diverifikasi pada: {{ $instructor->verified_at ? (\Carbon\Carbon::parse($instructor->verified_at)->format('d/m/Y')) : '-' }}</p>
                        </div>
                    </div>
                @endif
